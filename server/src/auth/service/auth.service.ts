...
export class AuthService {
  ...
  public async canCandidateConnect(token) {
    const candidate: { [key: string]: number } | any = this.jwtService.decode(token);
    const user = await this.userService.findOne(candidate.id);
    return user;
  }
